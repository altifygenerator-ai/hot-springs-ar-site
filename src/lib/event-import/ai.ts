...
import { cleanMultiline, cleanString, isFutureOrRecent, normalizeUrl } from "./utils";
...
function normalizeAiDraft(
  ...
  return {
    ...
    city: cleanString(record.city) || defaults.city || "Hot Springs",
    location_name:
    ...
    input: [
      {
        role: "system",
        content:
          "You extract event listings for a local Hot Springs, Arkansas tourism website. Return JSON only. Do not invent dates, times, prices, locations, performers, or links. Skip anything that is not clearly an upcoming event. Use YYYY-MM-DD dates and 24-hour HH:MM times when known. Mark needs_review true unless the listing is very clear.",
      },
      {
        role: "user",
...
Default city: ${source.city || "Hot Springs"}
    ...
    input: [
      {
        role: "system",
        content:
          "You turn pasted event text into one pending tourism event listing for Hot Springs, Arkansas. Return JSON only. Do not invent missing facts. Use needs_review true if details are missing or unclear. Use YYYY-MM-DD dates and 24-hour HH:MM times when known.",
      },
      {
        role: "user",
        content: `Today's date is ${today}.
