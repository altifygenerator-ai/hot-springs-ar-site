...
async function fetchSourceText(source: EventSource) {
  const response = await fetch(source.url, {
    headers: {
      "User-Agent":
        "HotSpringsArkansas.org event importer (+https://www.hotspringsarkansas.org)",
...
async function hasDuplicate(draft: ImportedEventDraft, sourceHash: string) {
...
export async function saveImportedEventDraft(
  ...
  const sourceHash = makeSourceHash(source?.id || null, {
    ...draft,
    title,
    start_date: startDate,
  });

  const duplicate = await hasDuplicate({ ...draft, title, start_date: startDate }, sourceHash);
  ...
  const eventRecord = {
    ...
    city: draft.city || source?.city || "Hot Springs",
...
export async function importEventSources(options: { sourceId?: string } = {}) {
...
export async function savePastedEvent(text: string, sourceUrl?: string | null) {
  const draft = await parsePastedEventWithAI(text, sourceUrl || null);
